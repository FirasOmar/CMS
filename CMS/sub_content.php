...
<th>View</th>
		...
		while($row=mysqli_fetch_array($result)){
			...
			if(!isset($array['en'][$sub_index]))
				$sub_index .= ".";
			foreach($array['en'][$sub_index] AS $subkey => $subitem)
			{	
					echo "<tr><td><input type='text' value='".$subkey."'>
					</td><td><form method='get' action='sub_sub_content.php'>
					<input name='go_to' type='submit' value='View' />
							<input name='sub_index' type='hidden' value='".$sub_index."' />
							<input name='sub_sub_index' type='hidden' value='".$subkey."' />
							<input name='id' type='hidden' value='".$i++."' />
								</form></td><td><form method='get'><input  type='submit' value='Edit' />
								</form></td></tr>";
			}
		}
		?>
